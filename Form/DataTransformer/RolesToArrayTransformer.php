...
namespace Instinct\Bundle\UserBundle\Form\DataTransformer;
...
class RolesToArrayTransformer implements DataTransformerInterface
{
    ...
    public function reverseTransform($roles)
    {
        ...
        $roleColection = $this->om
        ->getRepository('InstinctUserBundle:Role')
        ->findByArray($roles)
        ;

        if (is_string($roleColection))
        {
            throw new TransformationFailedException(sprintf(
                'An Role with string "%s" does not exist!',
                $roleColection
            ));

        }

        return $roleColection;
    }
}